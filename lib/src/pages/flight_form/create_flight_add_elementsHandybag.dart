...
import 'package:maando/src/blocs/elements_handybag_bloc_ad.dart';
...
class CreateFlightAddElementsHandybag extends StatefulWidget {
  CreateFlightAddElementsHandybag({Key key}) : super(key: key);

  @override
  _CreateFlightAddElementsHandybagState createState() =>
      _CreateFlightAddElementsHandybagState();
}

class _CreateFlightAddElementsHandybagState
    extends State<CreateFlightAddElementsHandybag> {
  @override
  void initState() {
  ...
  Widget build(BuildContext context) {
    conectivity.validarConexion(context, 'create_flight_add_elements_handybag');
    ...
    if (bloc.listElementsHandybagStreamValue == false) {
      elementsBlocHandybagAd.llenarListaElementosHandybag(context);
      elementsBlocHandybagAd.inicializarElementos();
    }
              ...
              Column(
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      ...
                      child: Text(
                        generalText.theElementsOnTheList(),
                        style: TextStyle(
                            fontSize: responsive.ip(2),
  ...
  Widget _filaDe5(Responsive responsive, List<Widget> elements, bloc) {
    ...
    for (var i = 0; i < bloc.listElementsHandybag.length; i++) {
      if (i <= 4) {
        ele = (bloc.listElementsHandybag[i]["name"].length < 9)
            ? _elementShort(responsive, bloc.listElementsHandybag[i], bloc)
            : _elementLong(responsive, bloc.listElementsHandybag[i], bloc);
  ...
  Widget _elementShort(Responsive responsive, dynamic element, bloc) {
    return StreamBuilder(
        stream: bloc.listElementsHandybagStream,
        builder:
          ...
          return CupertinoButton(
            ...
            onPressed: () {
              setState(() {
                for (var elem in bloc.listElementsHandybag) {
                  if (element["code"] == elem["code"]) {
                    elem["select"] = true;
                    bloc.changeListElementsHandybag(snapshopt.data);
                  }
                }
              });
            },
  ...
  _clear(BuildContext context, bloc) {
    setState(() {
