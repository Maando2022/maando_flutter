  ...
  Widget _timelineText(BuildContext context, double height, double width){
    ...
    return CupertinoButton(
      ...
      onPressed: (){
          ...
          if(state == generalText.atTheFrontDoor()){
            Http().updateStatusFlight(idFlight: this.flight["_id"], date: DateTime.now(), statusFlight: 'atTheFrontDoor')
            .then((value){
              Http().findFlight(idFlight: this.flight["_id"]).then((valueFinfFlight){
                if(json.decode(valueFinfFlight)["ok"]==true){
                  Navigator.pop(context);
                  setState(() {});
                  this.flight = json.decode(valueFinfFlight)["flightDB"];
                }
              });
            });
          }else if(state == generalText.takingOff()){
            Http().updateStatusFlight(idFlight: this.flight["_id"], date: DateTime.now(), statusFlight: 'takingOff')
            .then((value){
              Http().findFlight(idFlight: this.flight["_id"]).then((valueFinfFlight){
                if(json.decode(valueFinfFlight)["ok"]==true){
                  setState(() {});
          ...
          }else if(state == generalText.landing()){
            Http().updateStatusFlight(idFlight: this.flight["_id"], date: DateTime.now(), statusFlight: 'landing')
            .then((value){
              Http().findFlight(idFlight: this.flight["_id"]).then((valueFinfFlight){
                if(json.decode(valueFinfFlight)["ok"]==true){
                  setState(() {});
          ...
          }else if(state == generalText.atTheExitDoor()){
            Http().updateStatusFlight(idFlight: this.flight["_id"], date: DateTime.now(), statusFlight: 'atTheExitDoor')
            .then((value){
              Http().findFlight(idFlight: this.flight["_id"]).then((valueFinfFlight){
                if(json.decode(valueFinfFlight)["ok"]==true){
                  setState(() {});
          ...
          }else if(state == generalText.delivered()){
            Http().updateStatusFlight(idFlight: this.flight["_id"], date: DateTime.now(), statusFlight: 'delivered')
            .then((value){
              Http().findFlight(idFlight: this.flight["_id"]).then((valueFinfFlight){
                if(json.decode(valueFinfFlight)["ok"]==true){
                  setState(() {});
